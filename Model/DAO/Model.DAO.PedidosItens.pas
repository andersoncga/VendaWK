...
type
  TDAOPedidosItens = class(TInterfacedObject, iDAOEntidade<TPedidosItens>)
    ...
    public
      ...
      function Atualizar: iDaoEntidade<TPedidosItens>; overload;
...
begin
  ...
  try
    FConexao
      .SQL('update pedidos_itens set qtd=:qtd, preco_venda=:preco where numero_item=:id')
...
begin
  ...
  try
    FConexao
      .SQL('delete from pedidos_itens where numero_item=:id')
      .Params('id', Id)
      .ExecSQl;
  except on e: Exception do
...
begin
  ...
  FDataSet  :=
    FConexao
      .SQL('select pi.numero_item, pi.id_produto, pro.descricao, pi.qtd, pi.preco_venda, (pi.qtd * pi.preco_venda) as total ' +
           'from pedidos_itens pi ' +
           'inner join produtos pro on pro.codigo = pi.id_produto ' +
           'where pi.numero_pedido = :id')
      .Params('id', Id)
      .Open
      .DataSet;
end;
